...
export function session(req: AuthRequest, _res: Response, next: NextFunction) {
  ...
  if (token) {
    ...
    if (s) {
      ...
      if (s.user_id) req.user = one('SELECT * FROM users WHERE id=? AND active=1', s.user_id);
    }
  }
  next();
}
export function newSession(req: AuthRequest, res: Response, userId: string | null) {
